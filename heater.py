# Code modified from UW MISL's code
# https://github.com/uwmisl/pdclient/blob/master/pdclient/heater.py
#
# Uses a set of GPIO pins to do the PWM that drives heaters on the electrode board,
# instead of the PWM chip built on the PurpleDrop device
...
class TemperatureControl(object):
    ...
    """
    def __init__(self, client, channel_gains, ymax, kP, tI, tD, alpha_drop, k_drop, ambient_temp=None, pwm_freq=10):
        self.client = client
        ...
        # GPIO pin setup
        GPIO.setmode(GPIO.BOARD) # use board numbers, not names
        self.pins = [31, 33, 35, 37] # the 4 pins we will use for PWM
        
        # list for PWM objects
        self.pwms = []   
        for i in range(len(self.pins)) :
            # initialize pins and PWM objects
            GPIO.setup(self.pins[i], GPIO.OUT)
            self.pwms.append(GPIO.PWM(self.pins[i], pwm_freq))

        # start each PWM object with initial 0 duty cycle
    ...
    def stop(self):
        ...
        # Stop each PWM 
        for pwm in self.pwms:
            pwm.ChangeDutyCycle(0) # maybe unnecessary, but just in case
            pwm.stop() 
    ...
    def run(self):
        ...
        for i in range(N):
            ...
            # change value of duty cycle on PWM objects
            self.pwms[i].ChangeDutyCycle(outputs[i] * 100)
